  ...
  <section>
    <b-table :data="data" :loading="isLoading">
      <template slot-scope="props">
        ...
        <b-table-column field="modules" label="Modules">
          {{ props.row.modules }}
        </b-table-column>
...
export default {
  ...
  computed: mapState({
    data: ({ role: { items } }) => items,
    form: ({ role: { form } }) => form,
    isLoading: ({ role: { isLoading } }) => isLoading,
    isSubmitting: ({ role: { isSubmitting } }) => isSubmitting,
    isFormActive: ({ role: { form } }) => !!form
  })
};
</script>