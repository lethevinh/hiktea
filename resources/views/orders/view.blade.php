      ...
      @foreach($order->items as $index => $item)
        <tr>
          <td class="product-info">
            <div class="preview">
              <a target="_blank" href="{{ $item->toLink() }}">
                <img style="height: 100px;" src="{{ $item->product->image_url }}">
              </a>
            </div>
            <div>
              <span class="product-title">
                 <a target="_blank" href="{{$item->toLink()}}">{{ $item->product->title }}</a>
              </span>:
              <span class="sku-title">{{ $item->productSku->title }}</span>
            </div>
          </td>
          <td class="sku-price text-center vertical-middle">{{ number_format($item->price, 0) }} vnd</td>
          <td class="sku-amount text-center vertical-middle">x{{ $item->amount }}</td>
          <td class="item-amount text-right vertical-middle">{{ number_format($item->price * $item->amount, 0) }} vnd</td>
        </tr>
      @endforeach
    ...
    <div class="order-bottom">
      ...
      <div class="order-summary text-right">
        ...
        <div class="total-amount">
          ...
          <div class="value text-bold text-red">{{ number_format($order->total_amount,0) }} vnd</div>
        </div>
        <div>
          ...
          @if(isset($order->extra['refund_disagree_reason']))
            <div>
              <span>Refusal of refund reason：</span>
...
{{--            <a class="btn btn-primary btn-sm" href="{{ route('payment.alipay', ['order' => $order->id]) }}">Pay with Ali-Pay</a>--}}
            <!-- 把之前的微信支付按钮换成这个 -->
{{--            <button class="btn btn-sm btn-success" id='btn-wechat'>WeChat payment</button>--}}
          </div>
        @endif
        <!-- 支付按钮结束 -->
