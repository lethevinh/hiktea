@reset
Feature: Manage transplants
  In order to manage transplant
  As a user
  I need to be able to retrieve, create, update and delete them through the API.

  Scenario: Create a transplant report
    ...
    And I send a "POST" request to "/api/transplants" with body:
    """
    {
      "gender": "m",
  ...
  Scenario: Update a transplant
    ...
    And There are 1 sheets
    And There are 1 assignments
    And There are 2 transplants
    When I add "Content-Type" header equal to "application/ld+json"
    And I log in with email "admin@admin.com"
    And I add "Accept" header equal to "application/ld+json"
    And I send a "PUT" request to "/api/transplants/1" with body:
    """
    {
      "transplatedAt":"2011-01-01T00:00:00+00:00",
  ...
  Scenario: Retrieve the transplant list
    Given There is user with email "admin@admin.com" and roles "ROLE_ADMIN,ROLE_USER"
    And There are 1 farms
    And There are 1 units
    And There are 2 greenhouses
    ...
    And I log in with email "admin@admin.com"
    And I send a "GET" request to "/api/transplants"
    Then the response status code should be 200
    And the response should be in JSON
    And the header "Content-Type" should be equal to "application/ld+json; charset=utf-8"
    ...
    """
    {
      "@context":"/api/contexts/Transplant",
      "@id":"/api/transplants",
      "@type":"hydra:Collection",
      "hydra:member":[
        {
          "@id":"/api/transplants/1",
          "@type":"Transplant",
          "gender":"m",
          "transplantedAt":null,
          "plantsCount":0.99,
          "expectedTransplantAt":null,
          "expectedProcessOldSoilAt":null,
          "expectedPrepareNewSoilAt":null,
          "isApproved":false,
          "assignment":{
  ...
  Scenario: Throw errors when a transplant is invalid
    Given There is user with email "admin@admin.com" and roles "ROLE_ADMIN,ROLE_USER"
    And There are 1 farms
    And There are 1 units
    And There are 2 greenhouses
